...
export default function App() {
  ...
  const [newHistoricName, setNewHistoricName] = useState("");
  ...
  const addHistoricPlayer = async (name) => { if (!name.trim()) return; if (data.players.find(p => p.name === name.trim())) { showToast("Player già esistente!", "err"); return; } await persist({ ...data, players: [...data.players, { id: Date.now().toString(), name: name.trim(), teamId: "", teamYear: "0", active: false }] }); showToast("Player storico aggiunto!"); };
    ...
    <div style={{ fontFamily: "'Barlow Condensed', Arial, sans-serif", background: "#0d0d12", minHeight: "100vh", color: "#f0f0f0" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "28px 16px 80px 16px" }}>
        ...
        {page === "stats" && (() => {
          ...
          const historicPlayers = totalPlayers - activePlayers.length;

          const teamStats = data.teams.map(team => {
            const teamPlayers = data.players.filter(p => p.teamId === team.id);
            const teamActivePlayers = teamPlayers.filter(p => p.active !== false);

            // Calcola su eventi filtrati
            let totalPresenze = 0, totalScores = 0, totalScoreSum = 0;
            ...
            return { team, teamPlayers, teamActivePlayers, mediaPresenze, mediaPunteggio };
          });
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 28 }}>
                {[
                  ...
                  { label: "✅ Player attivi", val: activePlayers.length, color: "#22c55e" },
              ...
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "#111118", borderBottom: "2px solid #21212e" }}>
                      {["Team", "Player totali", "Player attivi", "Presenze medie/evento", "Punteggio medio/presenza"].map(h => (
                  ...
                  <tbody>
                    {teamStats.map(({ team, teamPlayers, teamActivePlayers, mediaPresenze, mediaPunteggio }, i) => (
                      <tr key={team.id} className="tr" style={{ borderBottom: "1px solid #1c1c28" }}>
                        ...
                        <td style={{ padding: "12px 14px", textAlign: "center", color: "#aaa", fontWeight: 700 }}>{teamPlayers.length}</td>
                        <td style={{ padding: "12px 14px", textAlign: "center", color: "#22c55e", fontWeight: 700 }}>{teamActivePlayers.length}</td>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              ...
              <div>
                ...
                <div className="card" style={{ marginBottom: 10 }}>
                  <p style={{ color: "#555", fontSize: 11, textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 10 }}>✅ Nuovo player attivo</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div className="card" style={{ marginBottom: 14, borderColor: "#555" }}>
                  <p style={{ color: "#555", fontSize: 11, textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 10 }}>📦 Nuovo player storico</p>
                  <div style={{ display: "flex", gap: 10 }}>
                    <input className="inp" placeholder="Nome player storico" value={newHistoricName} onChange={e => setNewHistoricName(e.target.value)} onKeyDown={e => e.key === "Enter" && addHistoricPlayer(newHistoricName).then(() => setNewHistoricName(""))} />
                    <button className="btn btn-ghost" onClick={() => addHistoricPlayer(newHistoricName).then(() => setNewHistoricName(""))}>Aggiungi</button>
                  </div>
                  <p style={{ color: "#444", fontSize: 11, marginTop: 8 }}>Nessun team richiesto — se rientra, prende il team dal file importato.</p>
                </div>
